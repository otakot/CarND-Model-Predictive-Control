 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int i = 0; i < N - 1; i++) {
      ...
      fg[0] += ACCELERATION_SMOOTHNESS * CppAD::pow(vars[a_start + i], 2);
    }

    // Minimize the value gap between sequential actuations.
    for (int i = 0; i < N - 2; i++) {
      fg[0] += STEERING_TRANSITION_SMOOTHNESS * CppAD::pow(vars[delta_start + i + 1] - vars[delta_start + i], 2);
